...
class PublisherlistController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $dataTypeContent->load('Strategies:name,status,date_start,date_end', 'Strategies.Campaign:id,name');
    ...
    public function edit(Request $request, $id)
    {
        ...
        $logged_user = User::where('id', '=', Auth::user()->id)->get();
        $user_organization_id = $logged_user[0]->organization_id;

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','user_organization_id'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$request->ajax()) {
            // add this "IF" on update functions that have a file uploader
               if(!$request->has('list_file') || $request->input('list_file')=='[]'){
                $request->merge([
                ...
                $request->merge([
                    'list_file' => UploaderHelper::formatData($request->input('list_file'),$slug,$data->list_file)
                ]);
            }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_validate')) {
            //format data. replace it into request and move files to the correct folder
            $request->merge([
            'list_file' => UploaderHelper::formatData($request->input('list_file'),$slug)
            ]);
