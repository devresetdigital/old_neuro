...
class Strategy extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $devices_names = array('','Windows Computer','Apple Computer','iPad','iPhone','iPod','Apple Device','Android Phone','Android Tablet','Other','ctv','ott','Linux computer','Android Computer','Windows Phone');
        $oss_names = array('','windows', 'windows 8', 'windows 10', 'macos','linux', 'android', 'ios','other','rokuos','tizen','Windows Phone','CHROMEOS');
        ...
        //STRATEGY Publisherlist
        $publisherlists_ids = array();
        $npl=0;
        $publisherlists = $this->StrategiesPublisherlist;
        $pbincexc = 3;
        foreach ($this->StrategiesPublisherlist as $val){
            $npl++;
            $publisherlists_ids[]=$val->publisherlist_id;
            $pbincexc=$val->inc_exc;
        }
        //INC EXC
        if($npl>0){ $publisherlists_incexc = $pbincexc; } else { $publisherlists_incexc = 3;}
        ...
        $pixels_inc_exc = $this->StrategiesDataPixel["inc_exc"];
   
        if($pixels_inc_exc == null){
            $pixels_inc_exc = 3;
        }
        ...
        $data_cpm = $this->StrategiesSegment["data_cpm"];

        if(count($selected_segments)==1 && $selected_segments[0]==0){
            $selected_segments = array();
        }
        $selected_segments_inc_exc = $this->StrategiesSegment != null ? $this->StrategiesSegment->inc_exc : 3;

        $segments = array('');


        //contextual data
        $selected_contextual= explode(",",$this->StrategiesContextuals["contextual_id"]);
  
        //$ncontextual = count($selected_contextual);
        if(count($selected_contextual)==1 && $selected_contextual[0]==0){
            $selected_contextual = array();
        }
        $selected_contextual_inc_exc = isset($this->StrategiesContextuals) ? $this->StrategiesContextuals->inc_exc : 3;
        ...
        if(isset($pacing_monetary_values[0]) && $pacing_monetary_values[0]==3){
            $pacing_monetary_values[1] =  0;
            $pacing_monetary_values[2] =  0;
        }

        if(isset($pacing_impression_values[0]) && $pacing_impression_values[0]==3){
            $pacing_impression_values[1] =  0;
            $pacing_impression_values[2] =  0;
        }
        if(isset($frequency_cap_values[0]) && $frequency_cap_values[0]==2){
            $frequency_cap_values[1] =  0;
            $frequency_cap_values[2] =  0;
        }
        //Prefix
        // die(env('WL_PREFIX'));
        ...
        $response = [
          ...
          //  $this->mergeWhen($cities[0]!="", [
            ...
            "publisherlists" => ["inc_exc"=> $publisherlists_incexc,"data"=> $publisherlists_ids],
            ...
            "pixels_lists" => ["inc_exc"=>  $pixels_inc_exc,"data" => $pixels[0]==0 ? [] : $pixels],
            ...
            "segments" => ["inc_exc"=> $selected_segments_inc_exc,"data" => $selected_segments],
            "segments_cpm" => $data_cpm,
            "contextual" => ["inc_exc"=> $selected_contextual_inc_exc,"data" => $selected_contextual],
