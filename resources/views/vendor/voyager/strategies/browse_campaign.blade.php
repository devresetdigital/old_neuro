...
@endphp
@php $idprefixed = $wlprefix+intval($campaignId); @endphp
...
@section('page_header')
    <div class="container-fluid">
        ...
        @can('add',app($dataType->model_name))
            <a href="{{ route('voyager.'.$dataType->slug.'.create') }}?campaign_id={{ $campaignId  }}" class="btn btn-success">
                <i class="voyager-plus"></i> <span>{{ __('voyager::generic.add_new') }}</span>
            </a>
            <a href="/admin/bulk/strategies" class="btn btn-success btn-add-new">
                <i class="voyager-plus"></i> <span>Bulk processing</span>
            </a>
            <!--<a type="button" class="btn btn-primary" data-toggle="modal" data-target="#duplicateStrategyModal">
...
<ol class="breadcrumb hidden-xs">
    ...
    <li class="active"><a href="#">Strategies Campaign</a>
                                </li>
</ol>
@stop
...
@section('content')
    <div class="page-content compass container-fluid">
        ...
        <ul class="nav nav-tabs">
            <li><a data-toggle="tab" onclick="document.location.href='/admin/campaigns/{{ $campaignId  }}/edit'" href="#"><i class="voyager-book"></i> Summary</a></li>
            <li {!! 'class="active"' !!}><a data-toggle="tab" href="#commands"><i class="voyager-lab"></i> Strategies</a></li>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    @if ($row->field=='id')
                                                        <div class="readmore"> <a target="_blank" href="/admin/strategies/{{$data->id}}/edit">{{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</a></div>
                                                    @else
                                        ...
                                        <td class="no-sort no-click" id="bread-actions">
                                            ...
                                            <ul class="dropdown-menu" role="menu">
                                                <li><a href="/admin/strategies/reports/{{$data->id}}">Reports</a></li>
                                                <li><a href="/admin/strategies/{{$data->id}}/edit">Edit</a></li>
                                                <li><a href="/admin/strategies/{{$data->id}}/export">Export</a></li>
    ...
    <!-- Modal 
    <div class="modal fade" id="duplicateStrategyModal" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Duplicate Strategy</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <div id="strategy-duplicate"></div>
          </div>
        </div>
      </div>-->
@stop
...
@section('javascript')
    ...
    <script>
        ...
        const clone = (id)=> {
            if (confirm('Are you sure you want to clone this strategy with all its targets?')){
                $(window).attr('location','/admin/cloneStrategy/'+id);
            }else{
                return false;
            }
        }                          
        ...
        let STRATEGIES = [];
        @foreach($dataTypeContent as $data)
          STRATEGIES.push({ value: '{{ $data->getKey() }}', label: '{{ $data->name }}' });
        @endforeach
        $(document).ready(function () {
            ...
            $('.changeStatus').click(function (e) {
                ...
                if(confirm('Are you going to change the status, do you want to continue?')){
                    $.post("/api/strategies/changeStatus/"+id ,{},function(data){},'json').done(function(data) {
                        location.reload()
    ...
    <script src="{{ asset('js/strategy-duplicate.js')}}"></script>
@stop
