                ...
                <ol class="breadcrumb hidden-xs">
                    ...
                    @for($i = 1; $i <= count(Request::segments()); $i++)
                        ...
                        @if(Request::segment($i) != ltrim(route('voyager.dashboard', [], false), '/') && !is_numeric(Request::segment($i)))
                            ...
                            @if($i < count(Request::segments()) & $i > 0 && array_search('database',Request::segments())===false && $dataTypeContent !== null)
                                <li class="active"><a
                                            href="{{ $breadcrumb_url }}{{  Request::segment($i) == 'strategies' ? '?campaign_id='.( !is_null($dataTypeContent->getKey()) ? $dataTypeContent->campaign_id : $_GET["campaign_id"]) : '' }}">{{ ucwords(str_replace('-', ' ', str_replace('_', ' ', Request::segment($i)))) }}</a>
                                </li>
                            @else
