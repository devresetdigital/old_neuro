                            ...
                            <table id="dataTable" class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        @endforeach
                                        <th class="">First flight</th>
                                        <th class="">Last flight</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    @if ($row->field=='id')
                                                        <div class="readmore"> <a target="_blank" href="/admin/campaigns/{{$data->id}}/edit">{{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</a></div>
                                                    @else
                                        ...
                                        <td class="no-sort no-click" >
                                            <span>{{ \Carbon\Carbon::parse($data->firstFlight)->format('m-d-Y h:i:s') }}</span>         
                                            <!-- <span>{{ ($data->firstFlight) }}</span>      -->
                                        </td>
                                        <td class="no-sort no-click" >
                                            <span>{{ \Carbon\Carbon::parse($data->lastFlight)->format('m-d-Y h:i:s') }}</span>     
                                            <!-- <span>{{ ($data->lastFlight) }}</span>      -->
                                        </td>
                                        <td class="no-sort no-click" id="bread-actions">
                                            ...
                                            <ul class="dropdown-menu" role="menu">
                                                <li><a href="/admin/campaigns/{{$data->id}}/edit">Edit</a></li>
                                                @php
                                                ...
                                                @endphp
                                                <li><a href="/admin/creports?campaign_id={{$data->id}}">Report</a></li>
                                                <li><a href="/admin/campaigns/{{$data->id}}/export">Export</a></li>
...
@section('javascript')
    ...
    <script>
       const clone = (id)=> {
            if (confirm('Are you sure you want to clone this campaign and all its strategies?')){
                $(window).attr('location','/admin/cloneCampaign/'+id);
            }else{
                return false;
            }
        }      
        $(document).ready(function () {
            ...
            $('.changeStatus').click(function (e) {
                ...
                if(confirm('Are you going to change the status, do you want to continue?')){
                    $.post("/api/campaigns/changeStatus/"+id ,{},function(data){},'json').done(function(data) {
                        location.reload()
