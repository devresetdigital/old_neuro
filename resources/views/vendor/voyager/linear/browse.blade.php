                                    ...
                                    <tr>
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @include('voyager::multilingual.input-hidden-bread-browse')
                                                    @if ($row->field=='id')
                                                        <div class="readmore"> <a target="_blank" href="/admin/{{$dataType->slug}}/{{$data->id}}/edit">{{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</a></div>
                                                    @else
                                                ...
                                                @elseif($row->type == 'file' && !empty($data->{$row->field}) )
                                                    ...
                                                    @if(json_decode($data->{$row->field}))
                                                        @foreach(json_decode($data->{$row->field}) as $file)
                                                            @php
                                                            $filename =  public_path() .'/storage/'. $file->download_link;
                                                            @endphp
                                                            <a href="{{ Storage::disk(config('voyager.storage.disk'))->url($file->download_link) ?: '' }}" download>
                                                                {{ $file->original_name ?: '' }}  -  ({{ file_exists ($filename )  ? round((filesize($filename) /1024),2) . " Mb"  : 'unknown'}})
                                                            </a>
                                                            <br/>
                                                        @endforeach
                                                    @else
                                                        <a href="{{ Storage::disk(config('voyager.storage.disk'))->url($data->{$row->field}) }}" download>
                                                            Download
                                                        </a>
                                                    @endif
                                        ...
                                        <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    <li><a href="/admin/linear_report?campaign_id={{$data->id}}">Report</a></li>
                                                    <li><a href="/admin/{{$dataType->slug}}/{{$data->id}}/edit">Edit</a></li>
